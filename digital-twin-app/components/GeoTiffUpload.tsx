...
const initialSlot: SlotState = { busy: false, fileName: null, error: null };
...
export function GeoTiffUpload({ onUploaded, onCompareComplete, disabled }: GeoTiffUploadProps) {
  ...
  const uploadSlot = useCallback(
    async (slot: UploadSlot, file: File) => {
      ...
      setSlot({ busy: true, fileName: file.name, error: null });
      ...
      try {
        ...
        onUploaded({
          ...
          bounds: data.bounds ?? preview.bounds,
        });
        setSlot({ busy: false, fileName: file.name, error: null });
      } catch (error) {
        setSlot({
          busy: false,
          fileName: file.name,
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 shadow-xl backdrop-blur">
      ...
      <p className="mt-1 text-xs text-slate-400">
        Envie dois GeoTIFF georreferenciados. A comparação gera heatmap e pontos automáticos.
      </p>
      ...
      >
        {compareBusy ? "A processar comparação…" : "Comparar e gerar heatmap"}
      </button>
...
function UploadField({
  ...
  return (
    <label className="flex cursor-pointer flex-col gap-2 rounded-xl border border-dashed border-slate-600 bg-slate-950/50 p-3 transition hover:border-sky-400/60">
      ...
      <span className="text-[11px] text-slate-500">{slot} · .tif / .tiff</span>
      <input
        type="file"
