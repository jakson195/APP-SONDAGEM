...
export function fetchProjectAlerts(
  ...
  const qs = q.toString();
  return api(`/api/v1/projects/${projectId}/alerts${qs ? `?${qs}` : ""}`);
}

export function fetchAlertHistory(
  projectId: string,
): Promise<{ items: AlertItem[]; total: number }> {
  return api(`/api/v1/projects/${projectId}/alerts/history?limit=500`);
}

export function fetchAlertRules(projectId: string): Promise<AlertRules> {
  return api(`/api/v1/projects/${projectId}/alerts/rules`);
}
...
export function evaluateAlerts(
  ...
  return api(`/api/v1/projects/${projectId}/alerts/evaluate`, { method: "POST" });
}
...
export function patchAlertStatus(
  ...
  return api(`/api/v1/alerts/${alertId}`, {
    method: "PATCH",
...
export function fetchNotifications(
  ...
  return api(`/api/v1/projects/${projectId}/notifications${q}`);
}
...
export function markAllNotificationsRead(projectId: string): Promise<void> {
  return api(`/api/v1/projects/${projectId}/notifications/read-all`, {
    method: "POST",
  });
}
...
export function fetchCriticalAreasGeoJSON(
  ...
  return api(`/api/v1/projects/${projectId}/critical-areas/geojson`);
}
