...
export function DigitalTwinWorkspace() {
  ...
  const loadLastResult = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Nenhum resultado salvo. Execute a comparação primeiro.");
      }
      const data = (await response.json()) as CompareResult;
      handleCompareComplete(data);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Falha ao carregar resultado.");
    }
  ...
  return (
    <div className="grid min-h-0 flex-1 gap-4 p-4 lg:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="flex flex-col gap-4 overflow-y-auto">
        ...
        <button
          type="button"
          onClick={() => void loadLastResult()}
          className="w-full rounded-xl border border-slate-600 bg-slate-950/50 px-4 py-2 text-sm text-slate-200 transition hover:border-sky-400/50"
        >
          Carregar último resultado (heatmap + pontos)
        </button>
