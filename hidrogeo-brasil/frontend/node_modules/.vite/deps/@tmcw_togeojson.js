...
function extractProperties(ns, node) {
  ...
  for (const [n, url] of ns) {
    for (const child of Array.from(node.getElementsByTagNameNS(url, "*"))) {
      properties[child.tagName.replace(":", "_")] = (_a = nodeVal(child)) == null ? void 0 : _a.trim();
    }
  }
...
function getTrack(ns, node) {
  ...
  for (const segment of segments) {
    ...
    if (line) {
      ...
      if ((_a = line.times) == null ? void 0 : _a.length)
...
function* gpxGen(node) {
  ...
  const attrs = (_a = n.getElementsByTagName("gpx")[0]) == null ? void 0 : _a.attributes;
  if (attrs) {
    for (const attr of Array.from(attrs)) {
      if (((_b = attr.name) == null ? void 0 : _b.startsWith("xmlns:")) && attr.value !== GPXX_URI) {
...
function coordPair(node) {
  ...
  if (ll[0] === void 0 || Number.isNaN(ll[0]) || ll[1] === void 0 || Number.isNaN(ll[1])) {
    return null;
  }
...
function processAltitudeMode(mode) {
  switch (mode == null ? void 0 : mode.textContent) {
...
function getNetworkLink(node, styleMap, schema, options) {
  ...
  const geometry = ((_a = box == null ? void 0 : box.coordinateBox) == null ? void 0 : _a.geometry) || null;
  if (!geometry && options.skipNullGeometry) {
    return null;
  }
  const feature = {
    ...
    properties: Object.assign(
      ...
      extractTimeStamp(node),
      getLinkObject(node),
      (box == null ? void 0 : box.lod) ? { lod: box.lod } : {}
    )
  };
  if ((_b = box == null ? void 0 : box.coordinateBox) == null ? void 0 : _b.bbox) {
    feature.bbox = box.coordinateBox.bbox;
  }
  if (((_c = feature.properties) == null ? void 0 : _c.visibility) !== void 0) {
    feature.properties.visibility = feature.properties.visibility !== "0";
  }
  const id = node.getAttribute("id");
  if (id !== null && id !== "")
  ...
  function traverse(node2, pointer, options2) {
    if (isElement(node2)) {
      switch (node2.tagName) {
        ...
        case "NetworkLink": {
          const networkLink = getNetworkLink(node2, styleMap, schema, options2);
          if (networkLink) {
