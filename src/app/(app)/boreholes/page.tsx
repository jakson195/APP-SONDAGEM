...
export default function BoreholesPage() {
  ...
  useEffect(() => {
    ...
    setProjectId((prev) =>
      prev && projects.some((p: Project) => p.id === prev) ? prev : projects[0].id,
    );
  }, [projects]);

  const project = useMemo(
    () => projects.find((x: Project) => x.id === projectId),
    [projects, projectId],
  );
        ...
        {saved.length > 0 && (
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-sm">
            ...
            <ul className="divide-y divide-[var(--border)]">
              {saved.map((b) => {
                const savedProject = projects.find((p: Project) => p.id === b.projectId);
                return (
