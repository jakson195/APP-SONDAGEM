...
const VIEWER_READY = "hidrogeo-viewer-ready";

type Status = "checking" | "loading" | "ready" | "failed";

/** Iframe HidroGeo embutido no DataGeo — esconde overlay no load / postMessage. */
export function useHidroGeoIframeReady(viewerUrl: string) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      if (event.data?.type === VIEWER_READY) markReady();
    };
