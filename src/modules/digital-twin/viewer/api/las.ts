...
export function resolveTilesetUrl(tilesetPath: string): string {
  ...
  return `${base}${tilesetPath.startsWith("/") ? "" : "/"}${tilesetPath}`;
}
...
export async function uploadLas(
  ...
  const res = await fetch(
    `${base}/v1/projects/${pid(projectId)}/uploads/las`,
...
export async function fetchLasStatus(
  ...
  const base = resolveGeoApiBase();
  const res = await fetch(
    `${base}/v1/projects/${pid(projectId)}/uploads/las/${encodeURIComponent(terrainModelId)}/status`,
  );
  if (!res.ok) {
